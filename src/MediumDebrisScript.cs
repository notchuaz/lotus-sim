...
public class MediumDebrisScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Time.time > spawnTimer + 1.25f)
		{
			rb.gravityScale = 2f + Random.Range(-0.5f, 0.2f);
	...
	private void OnTriggerEnter2D(Collider2D collider)
	{
		...
		if (collider.gameObject.name.Equals("PlayerHitbox"))
		{
			...
			rb.velocity = new Vector2(0f, 0f);
			animator.SetBool("IsGrounded", value: true);
			PlayerInteractionManager.GetComponent<PlayerInteractionManager>().GetAttack(0.2f);
		}
	}
}
